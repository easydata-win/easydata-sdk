...
| `search_employees` | A people search across a list of companies - ids or names, one page. | 1 per row, never under 2 |
...
stub and a page of 100 is one request - with a floor of one credit per chunk on
`sales.search.employees`, because a chunk of companies that employ nobody your
search matches is still a request. **A failed lookup costs nothing.** The
