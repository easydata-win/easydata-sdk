...
test("the Go signer's own output verifies", () => {
  // A vector produced by the server's own webhook signer. Every other
  // test here signs with this file's own HMAC, which would keep passing if both
  // sides were wrong in the same way. Regenerate from the server's signing
  // function with the secret "whsec_cross_check" and the unix timestamp 1789000000.
  const body =
...
/** A Response whose body is the given SSE text, delivered in chunks. */
...
function streamingClient(responses: Response[]) {
  const seen: { url: string; headers: Record<string, string> }[] = [];
  const fetchImpl = (async (url: string | URL, init: RequestInit = {}) => {
    seen.push({ url: String(url), headers: { ...((init.headers ?? {}) as Record<string, string>) } });
    return responses.shift() ?? sseResponse('event: complete\ndata: {"status":"completed"}\n\n');
  }) as unknown as typeof globalThis.fetch;

  return {
    seen,
    client: new EasyData({ apiKey: 'pk_x', baseUrl: 'https://api.test/v1', fetch: fetchImpl }),
  };
}

test('stream yields entries and stops on complete', async () => {
  const { client, seen } = streamingClient([
    sseResponse(
      'id: c1\nevent: result\ndata: {"item_index":0,"status":"succeeded","credits_used":1}\n\n' +
        ': keepalive\n\n' +
        'id: c2\nevent: result\ndata: {"item_index":1,"status":"succeeded","credits_used":1}\n\n' +
        'id: c3\nevent: complete\ndata: {"batch_id":"b-1","status":"completed","cursor":"c3"}\n\n',
    ),
  ]);

  const got = [];
  for await (const e of client.stream('b-1')) got.push(e);

  assert.deepEqual(
    got.map((e) => e.item_index),
    [0, 1],
    'keepalive comments must not be parsed as entries',
  );
  assert.equal(seen[0]!.headers['Accept'], 'text/event-stream');
});

test('a chunk boundary mid-event does not split an entry', async () => {
  // The bug every naive SSE parser has: a `data:` line arriving across two TCP
  // reads gets parsed as half an entry. One byte at a time forces the issue.
  ...
  assert.equal(got[0]!.item_index, 7);
});

test('a dropped stream reconnects from the last event id', async () => {
  ...
  assert.deepEqual(got.map((e) => e.item_index), [0, 1]);
  assert.equal(seen.length, 2, 'the dropped connection was not re-opened');
  assert.equal(seen[1]!.headers['Last-Event-ID'], 'c1', 'resumed from the wrong position');
  assert.ok(seen[1]!.url.includes('cursor=c1'));
});

test('a timeout event transparently reconnects', async () => {
  // The server ending a long stream on purpose. A caller should never have to
  // handle it.
  const { client, seen } = streamingClient([
    sseResponse(
      'id: c1\nevent: result\ndata: {"item_index":0,"status":"succeeded","credits_used":1}\n\n' +
        'id: c1\nevent: timeout\ndata: {"cursor":"c1","reason":"stream duration limit"}\n\n',
    ),
  ...
  assert.equal(got.length, 1);
  assert.equal(seen.length, 2, 'a timeout did not reconnect');
});

test('an error event surfaces with the cursor to resume from', async () => {
  const { client } = streamingClient([
  ...
  await assert.rejects(
    async () => {
      for await (const _ of client.stream('b-1')) {
        /* consume */
      }
    },
    (e: unknown) => {
      assert.ok(e instanceof EasyDataError);
      assert.match(e.message, /c9/);
      return true;
    },
  );
});

test('a non-200 on the stream is the normal typed error', async () => {
  const { client } = streamingClient([
    new Response(JSON.stringify({ error: { type: 'not_found', message: 'no such batch' } }), {
      status: 404,
  ...
  await assert.rejects(
    async () => {
      for await (const _ of client.stream('missing')) {
        /* consume */
      }
    },
    (e: unknown) => {
